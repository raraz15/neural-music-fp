...
class SpecNCutout(tf.keras.layers.Layer):
    ...
    def get_random_hole_widths_heights(
        self, bsz, hole_minw, hole_maxw, hole_minh, hole_maxh
    ):
        """Randomize hole width and heights."""
        if hole_minw == hole_maxw:
            xs_w = hole_minw
        else:
            xs_w = tf.random.uniform(
                (bsz, self.n_holes), hole_minw, hole_maxw, tf.int32
            )  # (b,n): width of x for bth batch in nth hole

        if hole_minh == hole_maxh:
            ys_h = hole_minh
        else:
            ys_h = tf.random.uniform(
                (bsz, self.n_holes), hole_minh, hole_maxh, tf.int32
            )  # (b,n): height of y for bth batch in nth hole
        return xs_w, ys_h
    ...
    def call(self, x):
        if self.prob > 0:
            ...
            if self.uniform_mask:
                ...
                hole_act = tf.random.uniform([bsz], 0.0, 1.0)
                hole_act = tf.less(hole_act, self.prob)  # dtype: tf.bool, shape: N
            else:
                hole_act = tf.random.uniform([bsz, self.n_holes], 0.0, 1.0)
                ...
                for b in tf.range(bsz):
                    if hole_act[b]:
                        for n in tf.range(self.n_holes):
                            ...
                            self.varx[
                                b, ys_start[n] : ys_end[n], xs_start[n] : xs_end[n], :
                            ].assign(hole_area)
                    else:
                        pass
                        # bypass this patch
...
def test():
    ...
    spec_ncutout_layer = SpecNCutout(
        prob=0.5, n_holes=3, uniform_mask=False, hole_fill="random"
    )
