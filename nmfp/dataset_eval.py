"""This module contains the Dataset class for evaluating a model, which 
contains the loaders for the database and query tracks. Audio degradation 
is not possible with EvaluationDataset dataloaders. For degradation, 
check the dataset_creation/augment_query_chunks.py script."""

import os
import glob

from nmfp.dataloaders import GenerationLoader
...
class EvaluationDataset:
    ...
    def get_database_loader(
        ...
        """This loader will load the database tracks and segment them 
        according to the model configurations
        in the config file.
    ...
    def get_query_loader(
        ...
        """This loader will load the query chunks and segment them 
        according to the model configuration in the config file. We 
        use chunks of audio with the same duration for creating the 
        query dataset, but the loader can work with full tracks or 
        chunks with different durations too. They should have been 
        degraded previously.

        Parameters
        ----------
            query_chunks : str
                Directory containing the query chunks or a line 
                delimited text file containing paths.
            batch_size : int
                The batch size for the dataloader
            hop_duration : float
                The hop duration for the dataloader
        ...
        # Find the boundary files for segment-level evaluation
