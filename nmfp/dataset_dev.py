...
from nmfp.dataloaders import TrainLoader
    ...
    def _read_train_degradations(self):
        ...
        if self.tr_use_bg_deg:
            self.tr_bg_paths = sorted(
                glob.glob(self.tr_bg_root_dir + "**/*.wav", recursive=True)
            )
        ...
        if self.tr_use_rir_deg:
            self.tr_rir_paths = sorted(
                glob.glob(self.tr_rir_root_dir + "**/*.wav", recursive=True)
            )
        ...
        if self.tr_use_mir_deg:
            self.tr_mir_paths = sorted(
                glob.glob(self.tr_mir_root_dir + "**/*.wav", recursive=True)
            )
